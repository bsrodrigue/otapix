...
export default function AuthfForm({
  ...
  return (
    <form id={id} className={style.authform_container} onSubmit={onSubmit} method="POST">
      <div className={style.authform_header}>
        <h1>{title}</h1>
        <p>{comment}</p>
        <small>{subComment}</small>
      </div>
      <div className={style.authform_fields}>{children}</div>
      <div className={style.authform_footer}>
        ...
        <a className={style.authform_alternative} href={alternative?.[2]}>
          {alternative?.[0]}
          <span className="primary-color">&nbsp;{alternative?.[1]}</span>
        </a>
      </div>
    </form>
  );
